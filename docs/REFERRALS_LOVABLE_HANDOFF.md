...
8. Проверить три режима продукта: `inherit`, `custom`, `disabled`; убедиться, что снимок процента продажи не меняется задним числом.
9. Проверить скидку приглашённому: применяется более выгодная из существующей и реферальной, скидки не суммируются, `final_price` и `meta` согласованы.
10. Открыть `/r/{partner_code}`, завершить новую регистрацию в течение 60 дней и подтвердить first-referrer-wins.
11. Подтвердить, что уже существующий профиль не может самопривязаться по ссылке (`existing_profile_requires_admin`).
12. Включить только `is_enabled`, `tracking_enabled`, `partner_portal_enabled`, оставив `shadow_mode=true` и `accrual_enabled=false`.
13. Выполнить `referral_reconcile_orders(...)` сервисной ролью и сверить покупки, совершённые после `enabled_at`, но до первой привязки/авторизации реферала.
14. После shadow reconciliation отдельно согласовать включение реальных начислений.
...
- базовая валюта: `BYN`;
- комиссия и скидка имеют общие значения, которые суперадмин может менять;
- продукт наследует общие значения, переопределяет их либо исключается из программы;
...
- правило 60/40 доступно как выключенная по умолчанию опция суперадминистратора; при включении действует только для новых начислений;
