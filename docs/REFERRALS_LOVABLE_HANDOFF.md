...
7. Проверить partial/full refund и повтор одного provider event.
8. Открыть `/r/{partner_code}`, завершить регистрацию, подтвердить first-referrer-wins.
9. Включить только `is_enabled`, `tracking_enabled`, `partner_portal_enabled`, оставив `shadow_mode=true` и `accrual_enabled=false`.
10. После shadow reconciliation отдельно согласовать включение реальных начислений.
