...
По опубликованным условиям Google для Gemini 3.6 Flash доступен бесплатный
уровень, где стоимость входных и выходных токенов указана как `Free of
charge`. Это не означает безусловную бесплатность:

- бесплатный уровень имеет квоты и rate limits;
- доступность и лимиты зависят от проекта и могут изменяться;
- при переходе на paid tier тарифицируются входные и выходные токены;
- данные free tier могут использоваться Google для улучшения продуктов,
  тогда как для paid tier в таблице цен указано обратное.

Поэтому продукт не обещает пользователю «абсолютно бесплатно». На старте
используется free tier, а fallback сохраняет работу при исчерпании квоты.
Метрики `object_identification_source` и `fallback_reason` позволяют принять
решение о переходе на платный уровень по фактической нагрузке.
...
4. Синхронизировать exact SHA, применить только новую миграцию и развернуть
   только `asset-classifier`.
