...
4. Синхронизировать exact SHA и применить только серию из 14 managed-миграций:
   staging-документ остаётся неопубликованным, 12 идемпотентных батчей
   добавляют 2 349 узлов, финальная миграция проверяет checksum, уникальность
   якорей и только затем атомарно публикует документ, версию и текст сценария.
   После этого развернуть только `asset-classifier`.
