...
export function renderPilot(mode) {
  if (!['dry-run','rollback','execute'].includes(mode)) throw new Error('Explicit dry-run or execute mode required');
  ...
  const rendered=sql.replace(marker, mode !== 'dry-run' ? 'true' : 'false');
  return mode === 'rollback' ? rendered.replace(/COMMIT;\s*$/, 'ROLLBACK;\n') : rendered;
}
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const [, , mode, out] = process.argv;
  if (!out) throw new Error('Usage: node render-contact-merge-18-36.mjs <dry-run|rollback|execute> <output.sql>');
