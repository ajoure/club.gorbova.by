...
async function fixture(){
 ...
 created_at timestamptz DEFAULT now(),deal_date timestamptz,pipeline_id uuid,pipeline_stage_id uuid,meta jsonb,purchase_snapshot jsonb);
 ...
 CREATE TABLE access_grant_ledger(order_id uuid,source_order_id uuid);CREATE TABLE referral_balance_transactions(source_id uuid);`);
...
test('history dry-run is empty; paid facts preserve refs and create no money/access; repeat is zero',async()=>{
 const db=await fixture();try{
  ...
  for(const row of rows){assert.equal(row.status,'paid');assert.equal(Number(row.paid_amount),0);assert.equal(row.deal_date,null);assert.equal(row.meta.owner_confirmed_paid,true);assert.equal(row.provider,null);assert.equal(row.meta.deal_month,undefined);}
...
test('ordinary paid purchases retain automatic month assignment',async()=>{
 const db=await fixture();try{
  await db.query(`INSERT INTO orders_v2(id,profile_id,user_id,product_id,status,base_price,final_price,deal_date)
    VALUES('00000000-0000-4000-8000-000000000011',$1,$2,$3,'paid',123,123,'2026-08-15T12:00:00Z')`,[profile,user,courseId]);
  assert.equal((await db.query('SELECT meta FROM orders_v2')).rows[0].meta.deal_month,'2026-08');
 }finally{await db.close()}
});
