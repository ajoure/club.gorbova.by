...
try{
  ...
  const io=createManagedTransport({supabaseUrl:process.env.SUPABASE_URL,
    serviceKey:process.env.SUPABASE_SERVICE_ROLE_KEY});
  const actor=process.env.COURSE_KB_OWNER_ID,publicIo=createPublicCaptionTransport(),media=createGapMedia();
  let report;
  if(mode==='dry-run'){
    report=(await prepareHistoricalGapAudit(io,publicIo,actor,get('--event-id'),media)).manifest;
  }else{
    ...
    if(!info.isFile()||(info.mode&0o077)!==0||info.size>1000000)
      throw Error('private_manifest_required');
    const bytes=await readFile(path);
    if(sha(bytes)!==approvedHash)throw Error('manifest_hash_mismatch');
    const approved=JSON.parse(bytes.toString('utf8'));
    const captured=await prepareHistoricalGapAudit(io,publicIo,actor,approved.event?.id,media);
    report=await executeHistoricalGapAudit(io,publicIo,actor,approved,captured,
      createSttGateway(process.env.LOVABLE_API_KEY),async next=>{
        progress=next;await writeFile(output,JSON.stringify(next,null,2)+'\n',{mode:0o600});
      });
  }
  await writeFile(output,JSON.stringify(report,null,2)+'\n',{mode:0o600});
  process.stdout.write(JSON.stringify({ok:true,mode,parts:report.parts?.length,
    stt_calls:report.stt_calls,cached:report.cached,not_quality_approval:true})+'\n');
}catch(error){
  const code=/^[a-z_]+$/.test(error?.message||'')?error.message:'historical_gap_audit_stopped';
