...
test('full sync dry run, exact fingerprint apply, existing financial terms and idempotent rerun',async()=>{
 const {db,pairs}=await fixture();try{
  ...
  // The former 21-module override omitted three source modules. Compare the
  // transferred names as well as the count so unrelated target IDs cannot pass.
  const names=async(tariff,root)=>(await db.query("SELECT m.title FROM access_rules r CROSS JOIN LATERAL jsonb_array_elements_text(r.conditions->'allowed_module_ids') a(id) JOIN training_modules m ON m.id=a.id::uuid WHERE r.tariff_id=$1 AND r.target_ref=$2 ORDER BY m.title",[tariff,root])).rows.map(r=>r.title.toLocaleLowerCase('ru').replace(/(?:20|21) поток/g,'FLOW поток')).sort();
  assert.deepEqual(await names(pairs[0].target,root21),await names(pairs[0].source,root20));
