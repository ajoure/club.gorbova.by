...
import {prepareReviewedGapPublication,publishReviewedGap} from './lib/reviewed-gap-publication.mjs';
...
try{
  ...
  const prepared=await prepareReviewedGapPublication(io,createPublicCaptionTransport(),
  ...
  if(mode==='dry-run')report=prepared.manifest;
    ...
    report=await publishReviewedGap(io,process.env.COURSE_KB_OWNER_ID,approved,prepared,review,approvedHash);
  }
