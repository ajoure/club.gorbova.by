...
import {prepareHistoricalGapPublication,publishHistoricalGap} from './lib/historical-gap-publication.mjs';
...
try{
  ...
  const mode=get('--mode'),output=get('--report'),originalPath=get('--original-manifest'),
    reviewPath=get('--review-file');
  if(!['dry-run','execute'].includes(mode)||!output||!originalPath||!reviewPath)
    throw Error('historical_review_arguments_required');
  ...
  const prepared=await prepareHistoricalGapPublication(io,createPublicCaptionTransport(),
    actor,original,review,sha(reviewBytes));
  let report;
  if(mode==='dry-run')report=prepared.manifest;
    ...
    if(sha(bytes)!==approvedHash)throw Error('historical_review_manifest_hash_mismatch');
    const approved=JSON.parse(bytes.toString('utf8'));
    report=await publishHistoricalGap(io,actor,approved,prepared,review,approvedHash);
  }
  await writeFile(output,JSON.stringify(report,null,2)+'\n',{mode:0o600});
  process.stdout.write(JSON.stringify({ok:true,mode,stt_calls:0,paid_private:true})+'\n');
}catch(error){
  const code=/^[a-z_]+$/.test(error?.message||'')?error.message:'historical_review_stopped';
