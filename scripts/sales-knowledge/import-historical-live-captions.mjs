#!/usr/bin/env node
// Private historical event source. Never creates lessons or seller-facing facts.
...
try{
  ...
  if(!args.includes('--managed-lovable')||process.env.COURSE_KB_MANAGED_PROJECT_ID!=='796a93b9-74cc-403c-8ec5-cafdb2a5beaa')
    throw new Error('canonical_managed_environment_required');
  ...
  const actor=process.env.COURSE_KB_OWNER_ID,mode=get('--mode');let result;
  if(mode==='dry-run'){
    const eventId=get('--event-id');if(!eventId)throw new Error('historical_event_id_required');
    result=await dryRunHistoricalEvent(io,actor,eventId);
    ...
    result=await importHistoricalEvent(io,actor,JSON.parse(raw));
  }else throw new Error('mode_required');
  await writeFile(output,JSON.stringify(result,null,2)+'\n',{mode:0o600});
  process.stdout.write(JSON.stringify({ok:true,mode,ready:result.ready??true,stt_calls:0})+'\n');
}catch(error){
  const message=error instanceof Error?error.message:'';
  const code=/^[a-z_]+(?:_http_\d{3})?$/.test(message)?message:'historical_caption_import_failed';
  if(reportPath)await writeFile(reportPath,JSON.stringify({status:'stopped',code})+'\n',{mode:0o600}).catch(()=>{});
