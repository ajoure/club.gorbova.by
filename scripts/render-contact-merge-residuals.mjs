...
export function renderResidual(operation,mode) {
  if (!['G1','G10'].includes(operation) || !['dry-run','execute'].includes(mode)) throw new Error('Reviewed operation and explicit mode required');
  let sql=readFileSync(new URL('./contact-merge-residuals.sql',import.meta.url),'utf8');
  for(const [marker,value] of [["/* OPERATION */ 'G1'",`'${operation}'`],['/* EXECUTE_FLAG */ false',mode==='execute'?'true':'false']]) {
...
if(process.argv[1] && import.meta.url===pathToFileURL(process.argv[1]).href) {
  ...
  if(!out)throw new Error('Usage: node render-contact-merge-residuals.mjs G1|G10 dry-run|execute output.sql');
