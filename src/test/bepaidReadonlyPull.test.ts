...
import { requestHasServiceRoleKey } from '../../supabase/functions/_shared/service-request-auth';
...
function harness(admin = false) {
  ...
  const client = { auth: { getUser }, rpc: vi.fn().mockResolvedValue({ data: admin }), from: () => { throw new Error('No database writes permitted'); } };
  runInNewContext(compiled, {
    ...
    require(path: string) {
      ...
      if (path.includes('service-request-auth')) return { requestHasServiceRoleKey };
  ...
  return { run, fetch, creds, getUser };
}
