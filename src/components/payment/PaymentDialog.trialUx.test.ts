...
describe("PaymentDialog trial UX", () => {
  ...
  it("uses the canonical verified registration gate before trial activation", () => {
    expect(source).toContain("<InlineAuthForm");
    expect(source).toContain("onAuthenticated={handleCheckoutAuthenticated}");
    expect(source).toContain('defaultTab={isTrial ? "signup" : "login"}');
    expect(source).not.toContain("customerPassword:");
    expect(source).not.toContain("trial account sign-in failed");
  });
});
