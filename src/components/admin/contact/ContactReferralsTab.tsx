...
import { Copy, History, Link2, Loader2, ReceiptText, UserPlus, Users } from "lucide-react";
...
type PickerMode = "partner" | "referred" | null;
...
export function ContactReferralsTab({ profileId }: { profileId: string }) {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      const [relationships, sales, entries] = await Promise.all([
        ...
        client.from("referral_sale_attributions").select("id, public_id, status, commission_minor, reversed_minor, created_at, metadata, products_v2(name)").eq("partner_id", partner.id).order("created_at", { ascending: false }),
  ...
  const attachHistorical = useMutation({
    mutationFn: async () => {
      ...
      const { data, error } = await (supabase.rpc as any)("referral_admin_attach_historical_profile", {
    ...
    onSuccess: (relationshipId) => {
      toast.success("Историческая рекомендация добавлена. Теперь выберите покупки для начисления.");
  ...
  return <div className="space-y-4">
    {data.partner ? <Card><CardHeader><CardTitle className="text-base flex items-center justify-between gap-2">Партнёр <Badge>{referralStatusLabel(data.partner.status)}</Badge></CardTitle></CardHeader>
      <CardContent className="space-y-3">
        ...
        <Button variant="outline" size="sm" className="gap-2" onClick={() => setPickerMode("referred")}><Link2 className="h-4 w-4" /> Добавить существующего клиента</Button>
      </CardContent>
    ...
    <Card><CardHeader><CardTitle className="text-base flex items-center justify-between gap-2">Кто рекомендовал</CardTitle></CardHeader><CardContent className="space-y-3">
      {data.referredBy ? <div className="rounded-md border p-3 text-sm"><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{data.referredBy.partner?.profiles?.full_name || data.referredBy.partner?.profiles?.email || data.referredBy.partner?.partner_code || "Партнёр"}</p><p className="text-xs text-muted-foreground">Связь создана {new Date(data.referredBy.attached_at).toLocaleDateString("ru-BY")}</p></div>{data.referredBy.metadata?.administrative_historical && <Badge variant="secondary">Внесено администратором</Badge>}</div>{data.referredBy.manual_reason && <p className="mt-2 text-xs text-muted-foreground">Основание: {data.referredBy.manual_reason}</p>}</div> : <><p className="text-sm text-muted-foreground">Рекомендатель не указан.</p><Button variant="outline" size="sm" className="gap-2" onClick={() => setPickerMode("partner")}><Link2 className="h-4 w-4" /> Указать рекомендателя</Button></>}
    </CardContent></Card>
    ...
    {(data.referredBy || historicalRelationshipId) && <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><History className="h-4 w-4" /> Исторические покупки</CardTitle><p className="text-sm text-muted-foreground">Выберите только подтверждённые покупки. Комиссия рассчитывается по действующим правилам продукта; автопродления не попадут в список.</p></CardHeader><CardContent className="space-y-2">
      ...
      {historicalOrders.data?.map((order: any) => <div key={order.order_id} className="flex flex-col gap-2 rounded-md border p-3 text-sm sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{order.product_name}</p><p className="text-xs text-muted-foreground">{order.order_number} · {new Date(order.created_at).toLocaleDateString("ru-BY")} · {order.payments_count} платёж(ей)</p><p className="mt-1 font-medium">{formatBynMinor(Number(order.paid_minor))}</p></div><Button size="sm" className="gap-1.5" disabled={!order.commissionable} onClick={() => setSelectedOrder(order)}><ReceiptText className="h-3.5 w-3.5" /> Начислить</Button></div>)}
      {!historicalOrders.isLoading && !historicalOrders.data?.length && <p className="py-4 text-center text-sm text-muted-foreground">Подходящих оплаченных покупок без начисления нет.</p>}
    </CardContent></Card>}
    ...
    {data.partner && <Card><CardHeader><CardTitle className="text-base">Начисления</CardTitle></CardHeader><CardContent className="space-y-2">
      {data.sales.map((sale: any) => <div key={sale.id} className="border rounded-md p-3 text-sm flex justify-between gap-3"><div><p className="font-medium">{sale.products_v2?.name || "Продукт"}</p><div className="mt-1 flex flex-wrap gap-1"><Badge variant="secondary">{referralStatusLabel(sale.status)}</Badge>{sale.metadata?.administrative_historical && <Badge variant="outline">Внесено администратором</Badge>}</div></div><span className="font-medium">{formatBynMinor(Number(sale.commission_minor) - Number(sale.reversed_minor))}</span></div>)}
    ...
    <ContactPickerDialog
      ...
      options={{ title: pickerMode === "partner" ? "Выберите рекомендателя" : "Выберите приглашённого клиента", helperText: "Будет создана ручная историческая связь с обязательной отметкой администратора." }}
    />
    <Dialog open={pickedContact !== null} onOpenChange={(open) => { if (!open) { setPickedContact(null); setPickerMode(null); setLinkReason(""); } }}>
      <DialogContent><DialogHeader><DialogTitle>Подтвердить историческую рекомендацию</DialogTitle><DialogDescription>Связь будет внесена администратором и не начислит покупки автоматически: их нужно выбрать отдельно.</DialogDescription></DialogHeader><div className="space-y-2"><Label>Основание</Label><Textarea value={linkReason} onChange={(event) => setLinkReason(event.target.value)} placeholder="Например: рекомендация подтверждена менеджером 25.07.2026" /></div><DialogFooter><Button variant="outline" onClick={() => { setPickedContact(null); setPickerMode(null); }}>Отмена</Button><Button disabled={attachHistorical.isPending || !linkReason.trim()} onClick={() => attachHistorical.mutate()}>Сохранить связь</Button></DialogFooter></DialogContent>
    </Dialog>
