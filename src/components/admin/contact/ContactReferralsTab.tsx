...
export function ContactReferralsTab({ profileId }: { profileId: string }) {
  ...
  return <div className="space-y-4">
    ...
    <Card><CardHeader><CardTitle className="text-base flex items-center justify-between gap-2">Кто рекомендовал</CardTitle></CardHeader><CardContent className="space-y-3">
      {data.referredBy ? <div className="rounded-md border p-3 text-sm"><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{data.referredBy.partner?.profiles?.full_name || data.referredBy.partner?.profiles?.email || data.referredBy.partner?.partner_code || "Партнёр"}</p><p className="text-xs text-muted-foreground">Связь создана {new Date(data.referredBy.attached_at).toLocaleDateString("ru-BY")}</p></div>{data.referredBy.metadata?.administrative_historical && <Badge variant="secondary">Внесено администратором</Badge>}</div>{data.referredBy.manual_reason && <p className="mt-2 text-xs text-muted-foreground">Основание: {data.referredBy.manual_reason}</p>}<div className="mt-3 flex flex-wrap gap-2"><Button variant="outline" size="sm" className="gap-1.5" onClick={() => { setRelationshipForCorrection(data.referredBy); setPickerMode("replace-partner"); }}><Pencil className="h-3.5 w-3.5" /> Изменить рекомендателя</Button><Button variant="outline" size="sm" className="gap-1.5 text-destructive hover:text-destructive" onClick={() => { setRelationshipForCorrection(data.referredBy); setRelationshipAction("revoke"); }}><Trash2 className="h-3.5 w-3.5" /> Отозвать рекомендацию</Button></div></div> : <><p className="text-sm text-muted-foreground">Рекомендатель не указан.</p><Button variant="outline" size="sm" className="gap-2" onClick={() => setPickerMode("partner")}><Link2 className="h-4 w-4" /> Указать рекомендателя</Button></>}
    </CardContent></Card>
    ...
    {(data.referredBy || historicalRelationshipId) && <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><History className="h-4 w-4" /> Исторические покупки</CardTitle><p className="text-sm text-muted-foreground">Выберите только подтверждённые покупки. Комиссия рассчитывается по действующим правилам продукта; автопродления не попадут в список.</p></CardHeader><CardContent className="space-y-2">
      ...
      {historicalOrders.data?.map((order: any) => <div key={order.order_id} className="flex flex-col gap-2 rounded-md border p-3 text-sm sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{order.product_name}</p><p className="text-xs text-muted-foreground">{order.order_number} · {new Date(order.created_at).toLocaleDateString("ru-BY")} · {order.payments_count} платёж(ей)</p><p className="mt-1 font-medium">{formatBynMinor(Number(order.paid_minor))}</p>{order.credit_action === "credited" && <Badge variant="secondary" className="mt-2">Уже начислено</Badge>}{order.credit_action === "restore" && <Badge variant="outline" className="mt-2">Исключено администратором</Badge>}</div>{order.credit_action === "credit" ? <Button size="sm" className="gap-1.5" disabled={!order.commissionable} onClick={() => setSelectedOrder(order)}><ReceiptText className="h-3.5 w-3.5" /> Начислить</Button> : order.credit_action === "restore" ? <Button size="sm" variant="outline" className="gap-1.5" onClick={() => setSaleCorrection({ saleId: order.sale_id, action: "restore", label: order.product_name })}><RotateCcw className="h-3.5 w-3.5" /> Восстановить</Button> : order.can_reverse ? <Button size="sm" variant="outline" className="gap-1.5 text-destructive hover:text-destructive" onClick={() => setSaleCorrection({ saleId: order.sale_id, action: "reverse", label: order.product_name })}><Trash2 className="h-3.5 w-3.5" /> Исключить</Button> : <Badge variant="secondary">{order.credit_action === "credited" ? "Начислено" : "Не участвует"}</Badge>}</div>)}
