...
> = {
  ...
  [POSTPONED_NO_SOT_SECTION_ID]: {
    hint: "Поля для исполнителя-физлица и ИП. Пока негде заполнить эти данные, поэтому не используйте их в шаблонах — подставится пусто.",
    helpTitle: "Нет источника данных",
    helpBullets: [
      "Сюда вынесены поля исполнителя-ФЛ, исполнителя-ИП и «организационно-правовая форма» — в карточке исполнителя для них пока нет полей ввода.",
      "Будут включены, когда в разделе «Исполнители» появятся соответствующие поля.",
      "Сейчас в шаблонах Word их использовать не стоит — подстановка вернёт пустую строку.",
    ],
  },
};
