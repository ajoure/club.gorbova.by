...
- **17 поток** — НЕ создаётся
- **16 поток** — НЕ создаётся
- **Бизнес-леди Сотрудник** (2 строки) — skip
- **Пустой тариф без модулей** (7 строк) — skip
- **bonus_skipped** — skip
- **renewal_skipped** — skip
- **99 no_flow_base_purchase после cut-off** — распределяем по 18/19 по датам, остаток → manual_review_after_flow_match
- **172 no_flow_base_purchase до cut-off** — импортируются с `flow_id = null`
- **Alias** — единственный разрешённый: `Перевозки → Грузо- и пассажироперевозки`
...
### 4A — Создать flow records (только 18 поток + 19 поток)

Создаются **только 2 flow records**. 16 поток НЕ создаётся. 17 поток НЕ создаётся.

Даты берутся **строго из patch3_flow_dry_run.txt** (source of truth):
...
potok-18:
  ...
  code        = potok-18
  name        = 18 поток
  start_date  = 2024-07-25   (из dry-run)
  end_date    = 2025-03-25   (из dry-run)
  is_active   = false
  is_default  = false
...
potok-19:
  ...
  code        = potok-19
  name        = 19 поток
  start_date  = 2025-06-13   (из dry-run)
  end_date    = 2025-07-21   (из dry-run)
  is_active   = false
  is_default  = false
  meta        = {"source": "patch4_historical_import_setup"}
```
...
Python-скрипт. Входы: `/tmp/deals.csv` + flow UUIDs из 4A. Выходы — 4 файла.
...
**Alias rule (единственный разрешённый в PATCH 4):**
```text
"Перевозки" → "Грузо- и пассажироперевозки"
child_product_id = 64d9f812-617c-41a8-b3dc-bb113156d6f3
public_id = PRD-000022
Никакого fuzzy matching модулей не вводится.
```

**Profile matching:**
- `source_user_id` → `profiles.external_id_gc` → `profile_id`
- Fallback: `email` → `profiles.email`
- Если не найден match → **unmatched**, строка НЕ импортируется
...
- `/mnt/documents/patch4_unmatched_profiles.csv` — unmatched profile строки

**Reconciliation guard:** import_ready + skipped + manual_review + unmatched = 1670. Если не сходится → STOP.
...
```text
order_number       = GC-{source_order_number} если заполнен, иначе GCID-{source_order_id}
                     Если GC-{number} уже занят но gc_deal_id другой → fallback GCID-{source_order_id}
product_id         = 7101ed3c-7839-4a74-ad95-aa0660369b22
...
base_price         = amount из CSV (= final_price)
final_price        = amount из CSV (= base_price)
...
**Дедупликация при INSERT (двухуровневая):**
1. Primary: `meta->>'gc_deal_id' = source_order_id` — если найден → skip как duplicate
2. Secondary: `order_number = GC-{source_order_number}` — если занят → fallback `GCID-{source_order_id}`
...
Extra (historical-specific):
  ...
  display_purchase_name       = для base_tariff: tariff_name + product_name
                                для модульных: из модулей (напр. "ЦБ 2.0: Учет у ИП")
...
**7 обязательных артефактов:**

1. `/mnt/documents/patch4_created_flows_proof.txt` — id, product_id, code, name, start_date, end_date, is_active, is_default, meta
...
5. `/mnt/documents/patch4_unmatched_profiles.csv` — unmatched profile строки
6. `/mnt/documents/patch4_import_batch_report.txt` — итоговые числа
7. `/mnt/documents/patch4_sample_orders_proof.txt` — proof по 4 типам строк
...
unmatched_profiles
reconciliation_total (must = 1670)
```

**patch4_sample_orders_proof.txt — минимум 4 примера:**
1. Обычная тарифная покупка с исходным flow (из CSV)
2. no_flow_base_purchase, назначенная в 18 по дате
3. no_flow_base_purchase, назначенная в 19 по дате
4. Модульная покупка с alias Перевозки
...
**НЕ создает:** subscriptions_v2, entitlements, доступы.
**Не меняет** существующие данные.
**Не меняет** существующие historical правила.
...
- Если reconciliation total ≠ 1670 → STOP и выдать reconciliation report
...
5. 7 proof-артефактов созданы
6. Итоговые числа зафиксированы в batch report
7. Reconciliation total = 1670

## Риски и зависимости

- Profile matching зависит от `external_id_gc` в profiles (8149 записей). При отсутствии match → unmatched bucket
- 99 строк no_flow_base_purchase: часть может попасть в manual_review_after_flow_match если created_at вне диапазонов 18/19
- Alias `Перевозки` — единственный разрешённый, касается ровно 2 строк
