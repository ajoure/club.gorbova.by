...
**Статус**: Ожидает ручной invoke

## Goal

Invoke debug mode after hardening deploy to get audit records with `debug=true` и `ttl_hint='debug'`, then verify all DoD criteria.
...
Ожидаемый ответ: `ok=true, mode="debug", userHasSBS=true, dryRun=true` (допускаются доп. поля: `via`, `orphanPsCount`, `executionId`).
...
Ожидание: 1 запись, `debug='true'`, `ttl_hint='debug'`, `source='manual_orphan_dod'`.

---

## Step 3 — DoD SQL-пруфы (4 запроса)

> Все запросы фильтруются по `created_at >= '<hardening_anchor_utc>'` (значение из Step 2).
...
WHERE meta->>'user_id' = '252e4b5c-8784-4876-a4ce-412444753b3a'
  ...
  AND created_at >= '<hardening_anchor_utc>'
ORDER BY created_at DESC
LIMIT 20;
```
...
| Условие | Действие |
|---------|----------|
| После invoke нет `orphan_dod_completed` с `debug=true` | Hardening НЕ пройден. Повторить invoke (макс. 3 попытки). После 3 — проверить секрет, headers, endpoint. |
...
| Hardening anchor | `orphan_dod_completed` | `debug='true'`, `ttl_hint='debug'`, `source='manual_orphan_dod'` |
| Мониторинг 22.03 | `paylink_cta_suppressed_sbs` | ≥1 запись (prod, без debug) |
