...
### Фаза 4 — Улучшения UX уведомлений
- [ ] **TimecodeInput** — маска ввода таймкода (HH:MM:SS) с валидацией
- [ ] **Автовыбор бота** — если бот один, выбирать автоматически
- [ ] **Выбор аудитории** — "только с доступом" / "всем пользователям"
- [ ] **Варианты времени** — "сейчас" / "при открытии доступа" / "анонс"
- [ ] **Кнопка "Тест"** — отправить тестовое сообщение себе
- [ ] **URL club.gorbova.by** — заменить gorbova.lovable.app
- [ ] **Уникальность slug** — проверка перед созданием урока

### Фаза 4.1 — Использование стилевого профиля Екатерины
- [ ] **Загрузка style_profile** из `telegram_publish_channels.settings`
- [ ] **Передача в промпт** — тон, writing_guidelines, characteristic_phrases
- [ ] **Fallback** — если профиль не найден, использовать базовый промпт

### Фаза 5 — UI покупки (следующий спринт)
...
- [ ] Убрать из промптов "весы, здания, документы"

---

## Архитектура генерации уведомлений

```
┌──────────────────────────────────────────────────────────────────┐
│                   СТИЛЕВОЙ ПРОФИЛЬ                               │
├──────────────────────────────────────────────────────────────────┤
│  telegram_publish_channels.settings.style_profile               │
│  ├─ tone: "экспертный, ироничный, наставнический"               │
│  ├─ writing_guidelines: ["Правило 1", "Правило 2", ...]         │
│  ├─ characteristic_phrases: ["фраза 1", "фраза 2", ...]         │
│  ├─ emojis: { used: true, frequency: "умеренно" }               │
│  └─ tone_details: "Описание стиля..."                           │
│                                                                  │
│  generate-lesson-notification                                    │
│  ├─ Получает style_profile из БД                                │
│  ├─ Формирует промпт с writing_guidelines                       │
│  └─ Генерирует текст в стиле Екатерины                         │
└──────────────────────────────────────────────────────────────────┘
```
