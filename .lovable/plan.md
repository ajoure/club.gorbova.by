...
| H2.1c-i legacy retirement patch | **pending** | retirement-only (НЕ bridge); удаление direct access writes из zone 2 + audit-only ответ |
...
| H4 `BEPAID_REBILL_MATERIALIZATION=mode=on` | **pending** | блокирован до закрытия H2.1c-i |
...
- До закрытия **H2.1c-i** (retirement patch) `BEPAID_REBILL_MATERIALIZATION=on` запрещён.
