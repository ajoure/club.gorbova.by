...
- [x] PR554 Publish plan: 13 critical/error findings классифицированы как pre-existing/out-of-scope; доступный GitHub-коннектор отсутствует, предоставленные run-результаты учтены; gate PASS без Publish.
- [x] Мобильный контакт-центр: убрать лишние строки в iPhone PWA, уплотнить историю и composer при клавиатуре; проверить Preview на mobile/desktop без отправки сообщений.
...
## Продукты3 / 17:87 (13.09.2026)
- [x] PLAN-ONLY ревизия Codex `admin_confirm_historical_cb_date` (commit da216e0de) — PASS, отчёт выдан
- [ ] Execute (отдельная команда после CI/merge точного SHA): managed migration → dry-run → rollback → execute → replay → read-back
