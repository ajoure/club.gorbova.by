...
-- Update get_inbox_dialogs_v1 to include guest profiles (where profile.user_id IS NULL)
-- For guests we store telegram_messages.user_id = profile.id (synthetic key, no FK on tm.user_id).
-- The search filter now matches by profiles.user_id OR profiles.id.
