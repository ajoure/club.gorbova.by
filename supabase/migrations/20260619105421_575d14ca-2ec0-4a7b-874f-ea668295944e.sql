...
-- v5 hotfix: kicked_at БЕЗ updated_at fallback; добавляем kicked_at_source
...
SELECT
  ...
  CASE WHEN p.user_id IS NULL THEN tcm.joined_chat_at ELSE COALESCE(
    (SELECT MIN(s.access_start_at) FROM public.subscriptions_v2 s
    ...
    (SELECT MIN(o.created_at) FROM public.orders_v2 o
  ...
  CASE
    ...
    ELSE COALESCE(
      (SELECT MAX(e.expires_at) FROM public.entitlements e
         JOIN public.product_club_mappings pcm ON pcm.product_id = e.product_id AND pcm.is_active = true
        WHERE e.user_id = p.user_id AND pcm.club_id = tcm.club_id),
      (SELECT MAX(s.access_end_at) FROM public.subscriptions_v2 s
         JOIN public.product_club_mappings pcm ON pcm.product_id = s.product_id AND pcm.is_active = true
        WHERE s.user_id = p.user_id AND pcm.club_id = tcm.club_id)
    )
  END AS access_ended_at,
  -- kicked_at: ТОЛЬКО из audit_logs, строгий per-member match, без updated_at fallback
  CASE
    WHEN tcm.access_status = 'removed' THEN
      (SELECT MAX(al.created_at)
         FROM public.audit_logs al
        WHERE al.action IN (
                'telegram.access_expired_revoke',
                'telegram.autokick.attempt',
                'AUTOKICK',
                'telegram.kick.manual'
              )
          ...
          AND COALESCE(al.meta->>'result', 'success') NOT IN ('failed','error','skipped','blocked'))
    ELSE NULL
  END AS kicked_at,
  CASE
    WHEN tcm.access_status = 'removed' AND EXISTS (
    ...
    WHEN tcm.access_status = 'removed' THEN 'unknown'
    ELSE NULL
  END AS kicked_at_source,
  (
...
-- RPC get_club_members_enriched (+ kicked_at_source)
...
BEGIN
  ...
  SELECT
    ...
    v.access_started_at, v.access_ended_at, v.kicked_at, v.kicked_at_source, v.is_commercial_orphan
...
-- RPC search_club_members_enriched (+ kicked_at_source)
