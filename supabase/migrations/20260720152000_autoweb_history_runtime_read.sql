-- Read-only source history for an authorised autoweb session. Direct client
-- reads of source live_stream tables are insufficient because a viewer can be
-- entitled to the replay event without having a separate source-event grant.
...
RETURNS TABLE(
  ...
  author_display_name text, author_role text, author_avatar_url text,
  author_nickname_color text
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT q.id, q.user_id, q.content, q.is_answered, q.answered_at, q.answered_by,
    q.created_at, q.author_display_name, q.author_role, q.author_avatar_url,
    q.author_nickname_color
