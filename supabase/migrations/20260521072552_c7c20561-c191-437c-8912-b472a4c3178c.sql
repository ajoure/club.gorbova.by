...
BEGIN
  ...
  IF v_existing_refund_id IS NOT NULL THEN
    RETURN jsonb_build_object('success', true, 'idempotent', true, 'refund_payment_id', v_existing_refund_id);
  END IF;
  ...
  UPDATE orders_v2
  SET status = v_new_order_status::order_status,  -- PATCH-REFUND-SOT-RPC-RECOVERY-2026-05: explicit cast text->enum
      meta = v_meta,
      updated_at = now()
  WHERE id = p_order_id;
