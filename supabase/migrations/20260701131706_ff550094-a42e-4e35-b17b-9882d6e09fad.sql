...
-- PATCH-CONTACT-FEED-COLUMN-FIX
-- email_logs: contact_id → profile_id, from_addr/to_addr → from_email/to_email
-- telegram_messages: contact_id → user_id (через profiles.user_id), text → message_text, нет chat_id
-- crm_activity_log: contact_id/action/description/entity_type не существуют → user_id (через profiles.user_id), activity_type/title_snapshot/text_snapshot
...
BEGIN
  ...
  SELECT user_id INTO _profile_user_id FROM public.profiles WHERE id = _contact_id LIMIT 1;
  ...
  tg_src AS (
    SELECT m.id::text AS id, 'telegram'::text AS kind, m.created_at AS at,
      COALESCE(m.direction, 'in')::text AS title, m.message_text AS body,
      jsonb_build_object(
        'user_id', m.user_id,
  ...
  email_src AS (
    SELECT e.id, 'email'::text AS kind, e.created_at AS at,
      ...
      jsonb_build_object(
        'from_email', e.from_email, 'to_email', e.to_email,
