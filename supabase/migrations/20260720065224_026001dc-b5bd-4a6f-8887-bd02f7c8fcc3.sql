...
BEGIN
  ...
  -- Pick a real active admin profile as fixture actor.
  ...
  ----------------------------------------------------------------
  -- Fixture CLD #1: fresh org, unique UNP, expected happy path
  ----------------------------------------------------------------
  INSERT INTO public.client_legal_details (
    id, profile_id, legal_form, organization_name, unp,
    legal_address, country, created_by, updated_by,
    created_at, updated_at
  ) VALUES (
    gen_random_uuid(), v_profile_id, 'ООО',
    'PHASE4D_TEST_ORG_1_'||_run_tag, '9'||to_char(floor(random()*1000000000)::int,'FM000000000'),
    'г. Тест, тестовая 1', 'BY', v_actor, v_actor, now(), now()
  ) RETURNING id INTO v_fixture_cld;

  -- T1: enqueue happy path
  ...
  -- T2: duplicate enqueue must return same job id (idempotency on active dedup key)
  ...
  -- T4 (auth-neg-not-applicable inside SD; recorded as N/A here, tested via HTTP outside):
  v_result := jsonb_set(v_result, '{t,T4_http_auth_negatives}', to_jsonb('tested_via_http'::text));

  ----------------------------------------------------------------
  -- Fixture CLD #2: unsupported reason path → worker will skip as done
  ----------------------------------------------------------------
  INSERT INTO public.client_legal_details (
    id, profile_id, legal_form, organization_name, unp,
    legal_address, country, created_by, updated_by,
    ...
    gen_random_uuid(), v_profile_id, 'ООО',
    'PHASE4D_TEST_ORG_2_'||_run_tag, '9'||to_char(floor(random()*1000000000)::int,'FM000000000'),
    'г. Тест, тестовая 2', 'BY', v_actor, v_actor, now(), now()
  ...
  ----------------------------------------------------------------
  -- Fixture CLD #3: map-mismatch guard test (T12)
  -- Pre-seed map with WRONG company_id, then attempt writer → must abort/dead_letter.
  ----------------------------------------------------------------
  INSERT INTO public.client_legal_details (
    id, profile_id, legal_form, organization_name, unp,
    legal_address, country, created_by, updated_by,
    ...
    gen_random_uuid(), v_profile_id, 'ООО',
    'PHASE4D_TEST_ORG_3_'||_run_tag, '9'||to_char(floor(random()*1000000000)::int,'FM000000000'),
    'г. Тест, тестовая 3', 'BY', v_actor, v_actor, now(), now()
  ) RETURNING id INTO v_fixture_cld3;

  -- Pick any existing company as the "wrong" mapping target
  ...
  ----------------------------------------------------------------
  -- Worker simulation loop: claim + process + complete, capturing outcomes.
  ----------------------------------------------------------------
  DECLARE
    r jsonb;
      ...
      ELSE
        BEGIN
          ...
          IF v_writer_ret IS NOT NULL AND v_writer_ret->>'status' <> 'ok' THEN
            ...
            v_error := left(v_writer_ret::text, 300);
          ELSE
            v_status := 'done';
            v_error := NULL;
          END IF;
        EXCEPTION WHEN OTHERS THEN
          ...
          IF v_err_state IN ('42501','23503','22023','P0001') OR v_err_msg ILIKE '%company_id mismatch%' THEN
            v_status := 'dead_letter';
          ELSE
            v_status := 'retry';
          END IF;
  ...
  -- T3 idempotency re-run: after job1 done, re-enqueue same CLD/reason must
  -- produce a NEW job (completed rows are excluded from active dedup index),
  -- and writer second pass must be zero-delta.
  DECLARE
    v_job1_second uuid;
  ...
  -- Force rollback: raise exception carrying the JSON payload.
  RAISE EXCEPTION 'PHASE4D_REHEARSAL_FORCED_ROLLBACK::%', v_result::text;
END
$fn$;

REVOKE ALL ON FUNCTION public.crm_phase4d_rehearsal_replay(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.crm_phase4d_rehearsal_replay(text) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.crm_phase4d_rehearsal_replay(text) TO service_role;
