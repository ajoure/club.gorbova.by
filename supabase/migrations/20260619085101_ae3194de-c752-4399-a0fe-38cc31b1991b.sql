...
-- 1) Update view to include access dates derived from subscriptions_v2 + entitlements via product_club_mappings
...
SELECT
  ...
  -- NEW: первая дата начала доступа к продукту этого клуба
  CASE WHEN p.user_id IS NULL THEN NULL ELSE (
    ...
    WHERE s.user_id = p.user_id
      AND pcm.club_id = tcm.club_id
  ) END AS access_started_at,
  -- NEW: дата окончания доступа (NULL если ещё действует)
  CASE
...
-- 2) Recreate RPC get_club_members_enriched with new return columns
...
BEGIN
  ...
  SELECT
    ...
    v.access_started_at, v.access_ended_at
...
-- 3) Recreate RPC search_club_members_enriched
