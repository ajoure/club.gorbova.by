...
-- PATCH-B: Update with_access_total and with_access_regular/with_access_admins to exclude removed
-- Anti-contradiction rule: removed users don't appear in "with_access" even if has_active_access=true
...
BEGIN
  ...
  FROM telegram_clubs tc
  LEFT JOIN v_club_members_enriched v ON v.club_id = tc.id
  LEFT JOIN LATERAL (
    SELECT
      CASE
        WHEN (v.last_telegram_check_result->'chat'->>'status') IN ('administrator','creator')
          OR (v.last_telegram_check_result->'channel'->>'status') IN ('administrator','creator')
        THEN true
        ELSE false
      END AS is_admin
  ) ac ON true
