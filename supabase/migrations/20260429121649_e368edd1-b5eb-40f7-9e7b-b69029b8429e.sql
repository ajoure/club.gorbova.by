...
-- Safety check + archive + hard delete RPCs for tariffs and offers
...
BEGIN
  ...
  SELECT count(*) INTO v_subs_active FROM public.subscriptions_v2 WHERE tariff_id = p_tariff_id AND status IN ('active','trialing','past_due','redirecting');
...
BEGIN
  ...
  v_can_hard := (v_orders + v_links_active + v_recon + v_lecta) = 0;
...
BEGIN
  ...
  VALUES (v_actor, 'tariff_archive', 'tariff', p_tariff_id, jsonb_build_object('source','tariff_archive_rpc'));
...
BEGIN
  ...
  INSERT INTO public.audit_logs(actor_id, action, entity_type, entity_id, payload)
  VALUES (v_actor, 'offer_archive', 'tariff_offer', p_offer_id, jsonb_build_object('source','offer_archive_rpc'));
...
BEGIN
  ...
  INSERT INTO public.audit_logs(actor_id, action, entity_type, entity_id, payload)
  VALUES (v_actor, 'tariff_hard_delete', 'tariff', p_tariff_id, v_safety);
...
BEGIN
  ...
  INSERT INTO public.audit_logs(actor_id, action, entity_type, entity_id, payload)
  VALUES (v_actor, 'offer_hard_delete', 'tariff_offer', p_offer_id, v_safety);
