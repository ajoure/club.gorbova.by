...
DECLARE
  ...
  v_admin_claims text;
  v_svc_claims text := '{"role":"service_role"}';
  v_claimed jsonb;
...
BEGIN
  ...
  IF v_profile_id IS NULL THEN RAISE EXCEPTION 'phase4d: no admin'; END IF;
  ...
  INSERT INTO public.client_legal_details (id, profile_id, client_type, purpose, status, is_default, leg_org_form, leg_name, leg_unp, leg_address, created_at, updated_at)
    VALUES (gen_random_uuid(), v_profile_id, 'legal_entity','billing','active',false,'ООО','PHASE4D_1_'||_run_tag,'9'||to_char(floor(random()*1000000000)::int,'FM000000000'),'test 1',now(),now())
    RETURNING id INTO v_fixture_cld;
  INSERT INTO public.client_legal_details (id, profile_id, client_type, purpose, status, is_default, leg_org_form, leg_name, leg_unp, leg_address, created_at, updated_at)
    VALUES (gen_random_uuid(), v_profile_id, 'legal_entity','billing','active',false,'ООО','PHASE4D_2_'||_run_tag,'9'||to_char(floor(random()*1000000000)::int,'FM000000000'),'test 2',now(),now())
    RETURNING id INTO v_fixture_cld2;
  INSERT INTO public.client_legal_details (id, profile_id, client_type, purpose, status, is_default, leg_org_form, leg_name, leg_unp, leg_address, created_at, updated_at)
    VALUES (gen_random_uuid(), v_profile_id, 'legal_entity','billing','active',false,'ООО','PHASE4D_3_'||_run_tag,'9'||to_char(floor(random()*1000000000)::int,'FM000000000'),'test 3',now(),now())
  ...
  -- Fully materialize the claim result (SRF) into jsonb before processing.
  ...
  FOR v_row IN SELECT * FROM jsonb_array_elements(v_claimed) LOOP
    ...
    BEGIN
      ...
      IF v_writer_ret IS NOT NULL AND (v_writer_ret->>'status') IS DISTINCT FROM 'ok'
         AND (v_writer_ret->>'ok') IS DISTINCT FROM 'true' THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      IF v_err_state IN ('42501','23503','22023','P0001') OR v_err_msg ILIKE '%mismatch%' THEN
  ...
  -- T3: re-enqueue completed CLD → new job created (dedup does not block terminal history)
  ...
  -- Manually mark this second job done — we bypassed claim/complete pairing to isolate idempotency.
