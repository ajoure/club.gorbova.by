...
BEGIN
  ...
  IF v_profile_id IS NULL THEN
    RAISE EXCEPTION 'phase4d_rehearsal: no admin profile available';
  END IF;
  ...
  -- Fixtures inserted under service_role (bypasses RLS via SECURITY DEFINER)
  INSERT INTO public.client_legal_details (
    id, profile_id, client_type, purpose, status, is_default,
    leg_org_form, leg_name, leg_unp, leg_address, created_at, updated_at
  ) VALUES (
    gen_random_uuid(), v_profile_id, 'legal_entity', 'billing', 'active', false,
  ...
  -- Switch to admin JWT claims for enqueue RPC calls.
  ...
  -- Restore service_role for worker RPCs.
  PERFORM set_config('request.jwt.claims', v_svc_claims, true);

  DECLARE
    r jsonb; v_outcomes jsonb := '[]'::jsonb;
    ...
    LOOP
      ...
      ELSE
        BEGIN
          ...
          ELSE v_status := 'done'; v_error := NULL; END IF;
        EXCEPTION WHEN OTHERS THEN
          ...
          IF v_err_state IN ('42501','23503','22023','P0001')
             OR v_err_msg ILIKE '%company_id mismatch%' THEN v_status := 'dead_letter';
  ...
  -- T3: re-enqueue completed CLD (needs admin claims again)
  DECLARE
    v_job1_second uuid;
