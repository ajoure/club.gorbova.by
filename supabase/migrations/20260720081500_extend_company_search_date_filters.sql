-- Phase 6: extend the existing guarded Company list RPC with calendar date bounds.
-- The contract stays additive; unknown filter keys are still rejected.
...
BEGIN
    ...
    WITH base AS (
      SELECT c.id, c.public_id, c.full_name, c.short_name, c.unp_normalized,
       ...
       WHERE ($1 OR c.status <> 'merged')
         ...
         AND ($8::text IS NULL OR c.public_id ILIKE '%%'||$8||'%%' OR c.full_name ILIKE '%%'||$8||'%%'
              OR c.short_name ILIKE '%%'||$8||'%%' OR c.unp_normalized ILIKE '%%'||$8||'%%'
              OR c.email ILIKE '%%'||$8||'%%' OR c.phone ILIKE '%%'||$8||'%%')
    )
  ...
  EXECUTE v_sql INTO v_items USING v_incl_merged, v_country, v_profile, v_status, v_kind,
                                   v_created_from, v_created_to, v_q;
  RETURN v_items;
END $$;
