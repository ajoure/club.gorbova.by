...
CREATE OR REPLACE FUNCTION public.crm_bulk_move_deals(
  _deal_ids uuid[], _pipeline_id uuid, _stage_id uuid, _request_id uuid
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.crm_pipeline_stages WHERE id = _stage_id AND pipeline_id = _pipeline_id)
  ...
  WITH before_rows AS (
    SELECT id, pipeline_id AS old_pipeline_id, pipeline_stage_id AS old_stage_id
      FROM public.orders_v2 WHERE id = ANY(_deal_ids) AND is_deleted = false FOR UPDATE
  ), moved AS (
    UPDATE public.orders_v2 o
       SET pipeline_id = _pipeline_id, pipeline_stage_id = _stage_id, updated_at = now()
      FROM before_rows b WHERE o.id = b.id
  ...
  UPDATE public.crm_deal_action_batches
     SET affected_count=v_affected, skipped_count=v_requested-v_affected, result=v_existing WHERE id=v_batch_id;
  RETURN v_existing;
END;
$function$;
...
DECLARE
  v_uid uuid := auth.uid(); v_note_id uuid; v_profile_id uuid; v_company_id uuid;
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'not_authenticated' USING ERRCODE='42501'; END IF;
...
CREATE OR REPLACE FUNCTION public.deal_feed_list(
  _deal_id uuid, _types text[] DEFAULT NULL, _search text DEFAULT NULL,
...
CREATE OR REPLACE FUNCTION public.crm_bulk_create_deals(
  _source_type text, _source_ids uuid[], _pipeline_id uuid, _stage_id uuid,
  _responsible_user_id uuid DEFAULT NULL, _title_template text DEFAULT NULL, _campaign_key text DEFAULT NULL,
  _task_type_id uuid DEFAULT NULL, _task_title text DEFAULT NULL, _task_due_at timestamptz DEFAULT NULL,
...
DECLARE
  ...
  v_profile_id uuid; v_company_id uuid; v_user_id uuid; v_email text; v_phone text;
  v_subject_name text; v_product_id uuid; v_tariff_id uuid; v_amount numeric; v_currency text;
  v_new_id uuid;
