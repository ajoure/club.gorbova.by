...
--
-- Customer orders and the referral ledger remain append-only.  An administrator
-- can exclude a pending commission from the referral calculation, restore that
-- exact commission while it is still reversible, and revoke or reassign a
-- relationship.  Every action requires a reason and writes an audit event.
