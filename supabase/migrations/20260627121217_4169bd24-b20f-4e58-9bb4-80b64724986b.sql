-- ─── 1. CRM routing for T-000074 (guarded) ────────────────────────────
update public.tariff_offers
set meta = coalesce(meta, '{}'::jsonb) || jsonb_build_object(
  'crm_routing', jsonb_build_object(
    'pipeline_id', 'a0000001-0000-0000-0000-000000000001',
    'stage_on_pending', 'b0000001-0001-0000-0000-000000000001',
    'stage_on_success', 'b0000001-0001-0000-0000-000000000003',
    'stage_on_failed',  'b0000001-0001-0000-0000-000000000004'
  )
), updated_at = now()
where id = '7b939741-e941-4dbc-b820-803cd7f307bc'
  and offer_type = 'preregistration';

-- ─── 2. Atomic preorder writer ─────────────────────────────────────────
...
begin
  ...
  -- Load + validate offer
  select id, offer_type, is_active, amount, product_id, tariff_id, meta
  into v_offer
  ...
  if v_offer.offer_type <> 'preregistration' then
    raise exception 'offer_type_not_preregistration' using errcode = '22023';
  end if;
  if v_offer.is_active is not true then
    raise exception 'offer_inactive' using errcode = '22023';
  end if;
  if coalesce(v_offer.amount, 0) <> 0 then
    raise exception 'offer_amount_must_be_zero' using errcode = '22023';
  end if;
  ...
  if v_pipeline_id is null or v_stage_id is null then
    raise exception 'crm_routing_missing' using errcode = '22023';
  end if;

  -- Dedupe: same offer + email within last 24h, status in active set
  select cp.id as prereg_id,
         (cp.meta ->> 'order_id')::uuid as order_id
  into v_existing
  ...
  if found then
    return jsonb_build_object(
      'deduped', true,
      'preregistration_id', v_existing.prereg_id,
      'order_id', v_existing.order_id
    );
  end if;

  -- Insert course_preregistrations
  insert into public.course_preregistrations (
    ...
    'cb20_predzapis',           -- legacy compatibility fallback
    null,
    'preorder_form',
    true,
    'new',
  ...
  -- Insert orders_v2 (CRM-visible draft, anti-revenue)
  ...
  -- Back-link order_id into preregistration meta
...
revoke all on function public.create_preorder_deal_atomic(uuid, text, text, text, boolean, uuid, text) from public;
grant execute on function public.create_preorder_deal_atomic(uuid, text, text, text, boolean, uuid, text) to authenticated, anon, service_role;