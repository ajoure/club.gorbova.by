...
-- Метаданные назначения роли (`document_package_item_role_assignments.metadata`)
-- больше не перезаписываются целиком на каждом save. Сохраняем все
-- верхнеуровневые ключи (`custom`, `position_gender`, …); ключ `position`
-- обновляется по контракту: отсутствует в input → не трогаем; пустой → удаляем;
-- непустой → перезаписываем.
--
-- Сигнатура RPC, права доступа, RLS-политики, search_path не меняются.
...
BEGIN
  ...
  IF _role_assignments IS NOT NULL THEN
    FOR v_item IN SELECT * FROM jsonb_array_elements(_role_assignments) LOOP
      ...
      END IF;
      -- Stage 4 hardening: enforce person ownership matches session owner profile.
      -- Admins still bypass. NULL profile_id on the person row is treated as
      -- inaccessible to non-admin actors.
      ...
      -- ===== PATCH-DPIRA-METADATA-MERGE-V1 =====
      -- Читаем существующий metadata активной строки (если есть) и
      -- собираем merged-значение по контракту `position`. Никакие другие
      -- верхнеуровневые ключи (`custom`, `position_gender`, …) не трогаем.
      SELECT metadata
        INTO v_cur_meta
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'metadata_merge_patch', 'dpira_metadata_merge_v1'
    )
