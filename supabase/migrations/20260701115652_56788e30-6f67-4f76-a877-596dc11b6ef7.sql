-- Обновляем contact_feed_list: добавляем Telegram, Email, Deal-события; человеческие подписи
...
BEGIN
  ...
  tg AS (
    SELECT tm.id, 'telegram'::text AS kind, tm.created_at AS at,
      CASE tm.direction WHEN 'outbound' THEN 'Исходящее' WHEN 'inbound' THEN 'Входящее' ELSE 'Telegram' END AS title,
  ...
  emails AS (
    SELECT el.id, 'email'::text AS kind, el.created_at AS at,
      COALESCE(el.subject, CASE el.direction WHEN 'outbound' THEN 'Исходящее письмо' ELSE 'Входящее письмо' END) AS title,
  ...
  files AS (
    SELECT f.id,
      CASE WHEN f.mime_type LIKE 'audio/%' AND (f.name ILIKE 'voice%' OR f.name ILIKE '%.webm') THEN 'voice_note' ELSE 'file' END AS kind,
      f.created_at AS at,
