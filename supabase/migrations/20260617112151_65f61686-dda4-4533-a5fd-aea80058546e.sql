-- Stage 4 hardening: enforce that person_id in role assignments belongs to the
-- profile that owns the session (admins bypass). Without this, the SECURITY DEFINER
-- RPC silently accepted persons from other tenants.
CREATE OR REPLACE FUNCTION public.save_session_document_atomic(_session_id uuid, _package_template_item_id uuid, _field_values jsonb, _role_assignments jsonb, _expected_template_version_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
