...
--
-- The active-only partial unique index is the final safety net, but a
-- read-then-insert implementation can still surface a 23505 to the caller
-- when two billing writes arrive at the same time.  An advisory transaction
-- lock keeps the common path deterministic; ON CONFLICT remains as a second
-- guard for callers that race outside this function.
