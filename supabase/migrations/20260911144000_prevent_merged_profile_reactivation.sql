...
BEGIN
  ...
  -- A merged alias must never become a second account through signup/recovery.
  IF EXISTS (SELECT 1 FROM public.profiles WHERE lower(trim(coalesce(email,'')))=_email_norm
      AND merged_to_profile_id IS NOT NULL)
    AND NOT EXISTS (SELECT 1 FROM public.profiles WHERE lower(trim(coalesce(email,'')))=_email_norm
      AND merged_to_profile_id IS NULL AND user_id IS NULL
      AND coalesce(status,'') IN ('archived','imported','active','blocked')) THEN
    RAISE EXCEPTION USING ERRCODE='23505', MESSAGE='Account registration unavailable';
  END IF;
