-- =====================================================================
-- Stage 3R: manual payment canonical source of truth + failed-retry race
-- =====================================================================

-- 1) Partial-unique index for manual payment idempotency in payments_v2
CREATE UNIQUE INDEX IF NOT EXISTS payments_v2_manual_admin_idem_key
  ON public.payments_v2 ((meta->>'idempotency_key'))
  WHERE origin = 'manual_admin' AND coalesce(is_deleted,false) = false;

-- 2) admin_create_manual_payment_v1: канонический ручной платёж напрямую в payments_v2
...
BEGIN
  ...
  IF p_profile_id IS NOT NULL THEN
    ...
    END IF;
  END IF;
  v_effective_user_id := v_profile_user_id;
  ...
  v_meta := jsonb_build_object(
    ...
    'manual_details', jsonb_strip_nulls(jsonb_build_object(
      ...
      'bank_document_no', nullif(trim(coalesce(p_bank_document_no,'')), ''),
      'external_id', nullif(trim(coalesce(p_external_id,'')), ''),
      'purpose', nullif(trim(coalesce(p_purpose,'')), ''),
      'note', nullif(trim(coalesce(p_note,'')), ''),
      'related_order_id', p_related_order_id,
      'customer_email', nullif(trim(lower(coalesce(p_customer_email,''))), '')
    ))
  );
  ...
  IF v_existing_id IS NULL THEN
    -- Shouldn't happen: conflict without matching row. Race with delete/tomb.
...
-- 3) Stage 2R.3 fix: failed→processing race must be atomic exception-safe.
--    Заменяем последовательность EXISTS→UPDATE на UPDATE в EXCEPTION-блоке.
...
BEGIN
  ...
  IF v_inserted_key IS NULL THEN
    ...
    ELSIF v_reservation.state = 'failed' THEN
      -- Stage 3R: атомарный переход failed→processing. Гонка с другой активной
      -- резервацией на тот же (source, source_row_id) даст 23505 из партиал-
      -- уникального индекса; ловим здесь и возвращаем контролируемый статус.
      BEGIN
  ...
  IF p_raw_source = 'queue' THEN
    ...
    UPDATE public.payment_reconcile_queue
     ...
     WHERE id = p_payment_id;
  ELSE
    -- Stage 3R: сохраняем origin='manual_admin', если платёж — ручной. Не
    -- перезаписываем провайдера/origin, только линкуем к сделке.
    UPDATE public.payments_v2
       SET order_id = v_order_id,
           profile_id = p_profile_id,
           user_id = CASE WHEN v_is_ghost THEN p_profile_id ELSE v_profile_user_id END,
           meta = coalesce(meta, '{}'::jsonb) || jsonb_build_object(
             'linked_by_admin', p_actor_user_id,
             'linked_at', now(),
             'linked_idempotency_key', p_idempotency_key
           )
     WHERE id = p_payment_id;
...
COMMENT ON FUNCTION public.admin_create_manual_payment_v1 IS
  'Stage 3R: канонический ручной платёж напрямую в payments_v2 (origin=manual_admin). Идемпотентен через partial-unique index по meta.idempotency_key. Не создаёт queue-строк, не выдаёт доступ.';
