-- Переписываем resolve_broadcast_audience_user_ids:
-- режим 'purchased' переходит с subscriptions_v2 на orders_v2 как канонический источник.
-- Поведение active_access, exclude, club, channels — без изменений.
...
BEGIN
  ...
  -- NEW: purchased now resolves from orders_v2 (canonical commercial SoT).
  -- Resolves user_id via: orders_v2.user_id → profiles.id (via profile_id) → profiles.email (via customer_email).
  ...
  filtered_by_clubs AS (
    ...
    WHERE
      ...
      OR EXISTS (
        ...
        WHERE tcm.user_id = fbe.user_id
          AND tcm.club_id = ANY(_club_ids)
          AND (
            (_club_membership = 'current' AND tcm.is_active = true)
