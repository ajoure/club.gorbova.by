-- C5-H: Extend search_deal_rows to include ai_generated_documents.document_number in search blob.
-- Preserves: signature, return columns, SECURITY INVOKER, owner postgres, EXECUTE to PUBLIC.

-- Supporting partial index for active documents with assigned numbers.
...
CREATE FUNCTION public.search_deal_rows(
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      o.pipeline_id   AS o_pipeline_id,
      ...
      lower(
        ...
        coalesce(pr.code, '') || ' ' ||
        coalesce(t.name, '') || ' ' ||
        coalesce(docs.doc_numbers_blob, '')
      ) AS search_blob
    FROM orders_v2 o
...
ALTER FUNCTION public.search_deal_rows(text, uuid, timestamptz, timestamptz, text, integer, integer) OWNER TO postgres;
GRANT EXECUTE ON FUNCTION public.search_deal_rows(text, uuid, timestamptz, timestamptz, text, integer, integer) TO PUBLIC;