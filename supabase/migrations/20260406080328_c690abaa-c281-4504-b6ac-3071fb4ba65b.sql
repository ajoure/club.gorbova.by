-- Fix 1: Recreate get_deal_tab_counts with corrected enum value
...
-- Fix 2: Create search_deal_rows RPC with typed columns
...
BEGIN
  ...
  WHERE
    -- Preset filters (same logic as client-side)
    ...
    AND (p_date_from IS NULL OR o.deal_date >= p_date_from)
    AND (p_date_to IS NULL OR o.deal_date <= p_date_to)
    -- Search filter (same as get_deal_tab_counts)
    AND (
      p_search IS NULL
      OR o.order_number ILIKE '%' || p_search || '%'
      OR o.customer_email ILIKE '%' || p_search || '%'
