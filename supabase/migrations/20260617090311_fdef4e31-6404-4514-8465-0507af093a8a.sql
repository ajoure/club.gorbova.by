...
BEGIN
  ...
  -- Boundary: field must belong to session's package template
  SELECT package_template_id INTO v_field_pkg
  FROM document_package_field_catalog
  WHERE id = _field_catalog_id;
  ...
  -- Boundary: item must belong to session's package template
  SELECT package_template_id INTO v_item_pkg
  FROM document_package_template_items
  WHERE id = _package_template_item_id;
  ...
  -- Capture previous value (for audit) and delete
...
REVOKE ALL ON FUNCTION public.delete_session_field_value(uuid, uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.delete_session_field_value(uuid, uuid, uuid) TO authenticated;