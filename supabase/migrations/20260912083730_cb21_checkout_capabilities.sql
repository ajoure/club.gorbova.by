...
LANGUAGE sql STABLE SECURITY INVOKER SET search_path='' AS $$
 ...
 CASE WHEN (SELECT coalesce(sum(pm.amount),0) FROM public.payments_v2 pm
   WHERE pm.order_id=o.id AND NOT pm.is_deleted AND pm.status::text='succeeded' AND pm.amount>0)>=o.final_price THEN 'provider'
...
LANGUAGE plpgsql SECURITY INVOKER SET search_path='' AS $$
DECLARE op public.sales_checkout_operations; o public.orders_v2; p public.sales_campaigns;
BEGIN
 SELECT * INTO op FROM public.sales_checkout_operations WHERE token_hash=p_hash FOR UPDATE;
 ...
 SELECT sc.* INTO p FROM public.sales_campaigns sc JOIN public.sales_conversations c ON c.campaign_id=sc.id WHERE c.id=op.conversation_id;
