...
-- Audience resolver for mass broadcast filters.
-- Single source of truth used by UI preview and by edge functions
-- (telegram-mass-broadcast, email-mass-broadcast).
...
BEGIN
  -- Permission gate
  IF NOT public.has_permission(auth.uid(), 'entitlements.manage'::permission_code) THEN
  ...
  -- Include: OR-union across rules (user must satisfy at least one rule, or no rules at all)
  included AS (
          ...
          CASE COALESCE(rule->>'mode', 'purchased')
            WHEN 'active_access' THEN EXISTS (
              ...
              WHERE s.user_id = b.user_id
                AND s.status = 'active'
                AND (
                  (rule->>'product_id') IS NULL OR (rule->>'product_id') = ''
                  OR s.product_id = (rule->>'product_id')::uuid
                )
                AND (
                  jsonb_typeof(rule->'tariff_ids') <> 'array'
                  OR jsonb_array_length(rule->'tariff_ids') = 0
                  OR s.tariff_id = ANY(
                    ARRAY(SELECT (jsonb_array_elements_text(rule->'tariff_ids'))::uuid)
                  )
                )
            )
            ELSE EXISTS (
              ...
              WHERE o.user_id = b.user_id
                ...
                AND (
                  (rule->>'product_id') IS NULL OR (rule->>'product_id') = ''
                  OR o.product_id = (rule->>'product_id')::uuid
                )
                AND (
                  jsonb_typeof(rule->'tariff_ids') <> 'array'
                  OR jsonb_array_length(rule->'tariff_ids') = 0
  ...
  -- Exclude: AND-intersection of "NOT EXISTS" for each rule
  after_exclude AS (
...
GRANT EXECUTE ON FUNCTION public.resolve_broadcast_audience_user_ids(jsonb) TO authenticated, service_role;
...
BEGIN
  ...
  sample AS (
    ...
    FROM (
      SELECT i.user_id FROM ids i LIMIT 50
    ) sub
