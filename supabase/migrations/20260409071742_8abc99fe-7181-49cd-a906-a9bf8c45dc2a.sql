...
-- Drop and recreate to change return type (add is_active column)
DROP FUNCTION IF EXISTS public.get_user_section_access(uuid);
...
BEGIN
  ...
  -- Admin bypass: all sections accessible (including inactive)
  ...
  -- Regular user: return ALL sections (including inactive) with access resolution
  RETURN QUERY
  ...
  resolved AS (
    SELECT
      ...
      CASE
        -- Inactive sections: no access for regular users
