...
GRANT EXECUTE ON FUNCTION public.save_session_document_atomic(uuid, uuid, jsonb, jsonb, uuid)
  TO authenticated, service_role;

COMMENT ON FUNCTION public.save_session_document_atomic(uuid, uuid, jsonb, jsonb, uuid) IS
'Stage 2 PATCH-PACKAGE-CROSS-PARITY-V1: atomic per-item save fields+roles with desired-state roles, orphan + stale-version guards.';