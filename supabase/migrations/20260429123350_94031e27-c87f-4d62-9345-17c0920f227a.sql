...
BEGIN
  ...
  WHERE (tariff_id = p_tariff_id OR offer_id IN (SELECT id FROM public.tariff_offers WHERE tariff_id = p_tariff_id))
  ...
  WHERE matched_tariff_id = p_tariff_id
     OR matched_offer_id IN (SELECT id FROM public.tariff_offers WHERE tariff_id = p_tariff_id);
  ...
  v_can_hard := (v_orders + v_subs_total + v_links_active + v_recon) = 0;
...
BEGIN
  ...
  SELECT count(*) INTO v_orders FROM public.orders_v2 WHERE offer_id = p_offer_id;
  SELECT count(*) INTO v_links_active FROM public.payment_links WHERE offer_id = p_offer_id AND COALESCE(status,'active') NOT IN ('canceled','expired','revoked');
  ...
  v_can_hard := (v_orders + v_links_active + v_recon) = 0;
...
REVOKE EXECUTE ON FUNCTION public.tariff_delete_safety_check(uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.offer_delete_safety_check(uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.tariff_archive(uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.offer_archive(uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.tariff_hard_delete(uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.offer_hard_delete(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.tariff_delete_safety_check(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.offer_delete_safety_check(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.tariff_archive(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.offer_archive(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.tariff_hard_delete(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.offer_hard_delete(uuid) TO authenticated;