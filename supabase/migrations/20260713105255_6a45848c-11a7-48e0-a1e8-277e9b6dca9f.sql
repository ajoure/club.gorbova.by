...
-- =====================================================================
-- Stage 2R.2 — payment lineage inside admin_deal_reservations
-- =====================================================================

ALTER TABLE public.admin_deal_reservations
  ADD COLUMN IF NOT EXISTS payment_id uuid,
  ADD COLUMN IF NOT EXISTS provider_snapshot text,
  ADD COLUMN IF NOT EXISTS source_amount_snapshot numeric,
  ADD COLUMN IF NOT EXISTS source_currency_snapshot text;

CREATE INDEX IF NOT EXISTS idx_admin_deal_reservations_payment
  ON public.admin_deal_reservations(payment_id)
...
BEGIN
  ...
  IF v_inserted_key IS NULL THEN
    ...
    IF v_reservation.state = 'completed' AND v_reservation.order_id IS NOT NULL THEN
      -- Stage 2R.2: replay возвращает ТОТ ЖЕ payment, зафиксированный в резервации.
