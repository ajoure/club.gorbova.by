...
BEGIN
  ...
  -- Atomic reservation: ON CONFLICT DO NOTHING (без target) ловит и pk,
  -- и partial unique (source, source_row_id).
  ...
  IF v_inserted_key IS NULL THEN
    -- Conflict happened. Determine which.
    ...
    IF NOT FOUND THEN
      -- Conflict was on (source, source_row_id) under a different key.
  ...
  -- Lock source row and read canonical amount/currency
