...
BEGIN
  ...
  UPDATE public.payments_v2 p
    SET is_deleted = true, deleted_at = now(), deleted_by = p_actor_user_id,
        ...
        deletion_context = jsonb_build_object(
          ...
          'checksum', v_op.checksum, 'graph_checksum', v_op.graph_checksum,
  ...
  LOOP
    ...
    IF coalesce((v_recalc->>'ok')::boolean, false) IS DISTINCT FROM true THEN
      RAISE EXCEPTION 'recalc_failed: order=% payment=% detail=%',
        v_pair.oid, v_pair.pid, v_recalc::text USING ERRCODE = 'P0001', HINT = 'batch rollback';
    END IF;
  END LOOP;
  ...
  -- Exact-lineage revocation: values conform to ALL access_grant_ledger CHECK constraints.
  ...
  VALUES (
    ...
    jsonb_build_object(
      'operation_id', v_op.id, 'operation_type', v_op.operation_type,
      'payment_ids', v_ids, 'order_ids', v_order_ids,
      'orders_deleted', v_order_deleted, 'ledger_revoked_ids', v_revoked_ids,
      'before', v_audit_before, 'after', v_audit_after, 'recalc', v_recalc_all,
