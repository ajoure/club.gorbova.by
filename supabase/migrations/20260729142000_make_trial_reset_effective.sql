-- A reset must remove every repeat-trial guard for this user/product, or fail
-- explicitly.  The previous implementation could return success while an
-- active trial_blocks row or a trial order still blocked the next activation.
