...
DECLARE
  v_def text;
  v_array_append_count int;
  v_scalar_concat_hits int;
  v_anon bool;
  v_auth bool;
  v_srv  bool;
BEGIN
  ...
  IF v_def IS NULL THEN
    RAISE EXCEPTION 'billing corrective post-check: function drifted after apply';
  END IF;

  v_array_append_count :=
    (SELECT count(*) FROM regexp_matches(v_def, 'array_append\(', 'g'));
  IF v_array_append_count <> 24 THEN
    RAISE EXCEPTION 'billing corrective post-check: expected 24 array_append calls, got %', v_array_append_count;
  END IF;

  v_scalar_concat_hits :=
    (SELECT count(*) FROM regexp_matches(v_def, 'v_(changed|conflicts)\s*:=\s*v_(changed|conflicts)\s*\|\|', 'g'));
  IF v_scalar_concat_hits <> 0 THEN
    RAISE EXCEPTION 'billing corrective post-check: legacy scalar concat still present (% hits)', v_scalar_concat_hits;
  END IF;

  SELECT has_function_privilege('anon','public.crm_company_upsert_from_billing(uuid)','EXECUTE') INTO v_anon;
  SELECT has_function_privilege('authenticated','public.crm_company_upsert_from_billing(uuid)','EXECUTE') INTO v_auth;
  SELECT has_function_privilege('service_role','public.crm_company_upsert_from_billing(uuid)','EXECUTE') INTO v_srv;
