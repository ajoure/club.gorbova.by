-- PATCH-PREORDER-CONVERT-AUDIT-FIX
-- Replace convert_preorder_on_pay_atomic to use audit_logs.actor_user_id (real column)
-- instead of audit_logs.actor_id (does not exist) and surface unexpected audit
-- failures via RAISE WARNING instead of silent swallow. Business/idempotency
-- logic unchanged.
...
BEGIN
  ...
  IF v_prereg_id IS NOT NULL THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      EXCEPTION WHEN OTHERS THEN
        RAISE WARNING 'convert_preorder_on_pay_atomic: audit insert (preregistration_update_failed) failed: % / %', SQLSTATE, SQLERRM;
      END;
    END;
  END IF;

  -- Primary audit. Guarded so business flow never breaks, but no longer silent.
  BEGIN
    INSERT INTO public.audit_logs (action, actor_type, actor_user_id, actor_label, meta)
    VALUES (
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'convert_preorder_on_pay_atomic: audit_logs insert failed (% / %): paid_order_id=%, preorder_order_id=%',
                  SQLSTATE, SQLERRM, p_paid_order_id, v_preorder.id;
  END;
