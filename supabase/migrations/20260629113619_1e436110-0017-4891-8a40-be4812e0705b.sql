    ...
    SELECT
      ...
      CASE
        ...
        WHEN sr.rule_product_id IS NOT NULL
          AND EXISTS (SELECT 1 FROM user_subs us WHERE us.product_id = sr.rule_product_id)
          THEN true
        WHEN sr.rule_product_id IS NOT NULL
          AND EXISTS (SELECT 1 FROM user_ents ue WHERE ue.product_id = sr.rule_product_id)
          THEN true
        ELSE false
