...
export interface TranscribeOptions {
  ...
  base64: string;
  format: string;
  /** Тип контента: "call" = разговор, "voice_note" = голосовая заметка. */
  kind?: "call" | "voice_note";
}
...
export async function transcribeAndSummarize(opts: TranscribeOptions): Promise<TranscribeResult> {
  ...
  const transcript = await callGateway(apiKey, [
    { role: "system", content: transcribeSystem },
    {
      role: "user",
      content: [
        { type: "text", text: kind === "voice_note" ? "Расшифруй это голосовое сообщение полностью." : "Расшифруй этот телефонный разговор полностью." },
