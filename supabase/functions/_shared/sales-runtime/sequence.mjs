...
const VALUES = {
  ...
  purchase_ready: ['unknown', 'accepted', 'declined'],
};
...
client содержит отдельно заказы, доказательства платежей, доступы и активность эфиров. paid_status_without_payment_proof, zero_price_order, trial, refund_or_refund_review не подтверждают оплаченную покупку. Доступ сам по себе не подтверждает оплату. Льгота определяется только alumni_eligibility, не твоим выводом. Наличие заказа не доказывает, кто обучался. История ограничена доступными каноническими записями; отсутствие записи не доказывает, что клиент никогда не покупал и не посещал эфир. room_presence подтверждает сохранённые входы/heartbeat, не непрерывное присутствие; player_evidence — позиции плеера, не длительность просмотра. reported_completed_at не доказательство просмотра целиком. Комментарии/вопросы/реакции — отдельные взаимодействия. Не говори «вы посмотрели эфир» на этом основании и не заполняй слоты evidence ссылками на карточку: нужны слова клиента из переписки.
...
slots содержит experience, goal, feedback, year, barrier, format, interest, purchase_ready. Каждый слот: {"value": одно допустимое значение, "evidence": [индексы customer-сообщений из history]}.
...
reference_topics — проверенные описания исторических учебных тем для понимания терминов и контекста. Они не подтверждают состав продаваемого курса, доступ к записи, расписание или тариф. Не выбирай их как fact_ids, не отвечай по ним учебными решениями и не превращай их в обещания продукта.
...
function hasExplicitPurchaseDecision(context, evidence) {
  const latestCustomerIndex = context.history.findLastIndex(message => message.role === 'customer');
  const latest = String(context.history[latestCustomerIndex]?.text ?? '').trim();
  if (/(?:не\s+(?:готов|хочу|буду|планирую|решил)|пока\s+не|(?:^|\s)(?:но|сначала|если)(?:\s|$)|перед\s+этим)/iu.test(latest)) return false;
  return evidence.some(index => {
    ...
    return previousSeller.question_id === 'decision' && /^(?:да|готов[аы]?|оформляем|беру)[.!\s]*$/iu.test(text);
  });
}
...
export function planDialogueReply(context, raw) {
  ...
  if (q === 'decision' && a.intent === 'product_question' && a.question_type !== 'price') {
    const allowed = f => a.question_type === 'program' ? f.id === 'program'
      ...
      : a.question_type === 'tariff' ? f.kind === 'offer' && !Number.isFinite(f.price)
