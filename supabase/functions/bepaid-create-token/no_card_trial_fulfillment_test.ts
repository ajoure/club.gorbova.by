...
Deno.test("no-card trial does not report success before access grant succeeds", () => {
  const grantCall = source.indexOf("const grantRes = await grantTrialAccessAsService(");
...
Deno.test("retry repairs the original paid no-card trial instead of creating a duplicate", () => {
  ...
  const repairCall = source.indexOf("const repairGrant = await grantTrialAccessAsService(", priorOrder);
